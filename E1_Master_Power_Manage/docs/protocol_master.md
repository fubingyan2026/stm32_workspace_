...
| 方向 | CAN ID | 说明 |
|------|--------|------|
| 主电源板 → 主机 | 0x001, 0x011-0x0B1 | 系统状态 + 电池数据上报 |
...
| 0x011 | 电池基础 | 主电源板 → 主机 | feedback_select bit0 | 双电池 SOC + 容量 + 循环 |
| 0x021 | 电池电压 | 主电源板 → 主机 | feedback_select bit1 | 双电池电压 + 充电标志 |
| 0x031 | 电池电流 | 主电源板 → 主机 | feedback_select bit2 | 双电池电流 |
| 0x041 | 电池温度 | 主电源板 → 主机 | feedback_select bit3 | 双电池电芯温度 |
| 0x051 | 电池版本 | 主电源板 → 主机 | feedback_select bit4 | 双电池 HW/SW 版本 |
| 0x061 | 故障码 | 主电源板 → 主机 | feedback_select bit5 | 双电池详细故障 |
| 0x071 | 故障等级+预警 | 主电源板 → 主机 | feedback_select bit6 | 预警告警 + 严重等级 |
...
| | 4 | `bat1_online` | 电池1 在线 |
| | 5 | `bat_has_error` | 任一电池有故障 |
| | 6 | `bat2_online` | 电池2 在线 |
| | 7 | 保留 | |
...
| 字节 | 字段 | 单位 | 描述 |
|------|------|------|------|
| 0 | `bat1_soc` | 1% | 电池1 SOC |
| 1 | `bat2_soc` | 1% | 电池2 SOC |
| 2-3 | `bat1_capacity` | 256 mAh | 电池1 设计容量高16位 (÷256) |
| 4-5 | `bat2_capacity` | 256 mAh | 电池2 设计容量高16位 (÷256) |
| 6 | `bat1_cycle` | 次 | 电池1 循环次数低8位 |
| 7 | `bat2_cycle` | 次 | 电池2 循环次数低8位 |

---

## 0x021 — 电池电压 + 充电标志  [主电源板 → 主机]

| 字节 | 字段 | 单位 | 描述 |
|------|------|------|------|
| 0-1 | `bat1_voltage` | 0.1V (uint16 LE) | 电池1 电压 |
| 2-3 | `bat2_voltage` | 0.1V (uint16 LE) | 电池2 电压 |
| 4 | 充电标志 | bit0=bat1_charging, bit1=bat2_charging | |
| 5-7 | 保留 | | |
...
| 字节 | 字段 | 单位 | 描述 |
|------|------|------|------|
| 0 | `bat1_temp` | °C (int8) | 电池1 电芯温度 |
| 1 | `bat2_temp` | °C (int8) | 电池2 电芯温度 |
| 2-7 | 保留 | | |
...
| 字节 | 字段 | 单位 | 描述 |
|------|------|------|------|
| 0-1 | `bat1_current` | 0.1A (int16 LE) | 电池1 电流（正=放电） |
| 2-3 | `bat2_current` | 0.1A (int16 LE) | 电池2 电流（正=放电） |
| 4-7 | 保留 | | |
...
| 4 | | ← | 0x021 电池电压（因反馈请求 bit1） |
| 5 | | ← | 0x031 电池电流（因反馈请求 bit2） |
| 6 | | ← | 0x041 电池温度（因反馈请求 bit3） |
| 7 | | ← | 0x051 电池版本（因反馈请求 bit4） |
...
**0x001 系统状态帧**每 100ms 自动发送。电池数据帧（0x011-0x0B1）**按需上报**——仅当主机通过 0x001 控制帧指定 `feedback_select` 位时才回复对应帧。`feedback_select=0x00` 时只返回 0x001 状态帧。
...
| bit1 | `SRV_CAN_MST_FEEDBACK_BAT_VOLTAGE` | 0x021 |
| bit2 | `SRV_CAN_MST_FEEDBACK_BAT_CURRENT` | 0x031 |
| bit3 | `SRV_CAN_MST_FEEDBACK_BAT_TEMPERATURE` | 0x041 |
| bit4 | `SRV_CAN_MST_FEEDBACK_BAT_STATUS` | 0x051 |
| bit5 | `SRV_CAN_MST_FEEDBACK_BAT_ERROR` | 0x061 |
| bit6 | `SRV_CAN_MST_FEEDBACK_BAT_COUNTER` | 0x071 |
| bit7 | `SRV_CAN_MST_FEEDBACK_BAT_SERIAL` | 0x081-0x0B1 (保留) |
