...
| 主机 → 主电源板 | 0x001 | 反馈请求 + 蜂鸣器/输出控制 |
| 主机 → 主电源板 | 0x004 | RGB 输出控制 |
...
| 0x004 | RGB 输出控制 | 主机 → 主电源板 | 主机主动发送 | 双 LED RGB 亮度（索引 0-31=通道1, 32-63=通道2） |
...
| CAN ID | 帧名 | 方向 | 帧长度 | 说明 |
|--------|------|------|--------|------|
| 0x001 | 控制指令 | 主机 → 主电源板 | 3 字节 | 反馈请求 + 蜂鸣器/输出控制 |
| 0x004 | RGB 输出控制 | 主机 → 主电源板 | 8 字节 | 双 LED RGB 亮度（一帧控 2 灯） |
| 0x003 | 进入升级模式 | 主机 → 主电源板 | 1 字节 | 请求 App 置 upgrade_flag 复位进 Bootloader |

### 0x001 — 基础控制指令帧(不包括led)

| 字节 | 字段 | 描述 |
|------|------|------|
| 0 | `feedback_select` | 位掩码，选择请求哪些电池数据帧（见上文 feedback_select 表） |
| 1 | `buzzer_duty` | 蜂鸣器占空比 0-50 |
| 2 | `ctrl_byte`   | 输出控制位（见下表） |
...
| 字节 | 字段 | 描述 |
|------|------|------|
| 0 | `led1_index` | LED1 索引：0-31=通道1(RGB1/SPI1)，32-63=通道2(RGB2/SPI3) |
| 1 | `led1_r` | LED1 红亮度 0-255 |
| 2 | `led1_g` | LED1 绿亮度 0-255 |
| 3 | `led1_b` | LED1 蓝亮度 0-255 |
| 4 | `led2_index` | LED2 索引：0-31=通道1，32-63=通道2 |
| 5 | `led2_r` | LED2 红亮度 0-255 |
| 6 | `led2_g` | LED2 绿亮度 0-255 |
| 7 | `led2_b` | LED2 蓝亮度 0-255 |

**要点：**
- 一帧控制 **2 个 LED**，每个 LED 4 字节（索引 + RGB 亮度），无保留字节
- 索引超出对应通道 LED 数时忽略该灯
- 收到首帧 0x004 后灯带退出彗星动画，进入手动控制模式；后续帧覆盖对应 LED 并刷新
- 帧长度必须为 **8 字节**
