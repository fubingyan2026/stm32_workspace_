...
static void led_phys_write(srv_led_handle_t* handle, uint16_t value);
...
static void led_process_cmds(srv_led_handle_t* handle);
...
static fsm_state_t led_fsm_blink_handler(fsm_t* ctx)
{
    ...
    if (handle->blink_code_phase == SRV_LED_BLINK_PHASE_BLINKING) {
        ...
        if ((uint32_t)(now - handle->interval_start_time) >= handle->current_cmd.led_blink_wait_ms) {
            handle->blink_code_phase = SRV_LED_BLINK_PHASE_BLINKING;
            handle->last_toggle_time = now;
        }
    }
    return SRV_LED_STATE_BLINK_CODE;
}
...
static fsm_state_t led_fsm_breathing_handler(fsm_t* ctx)
{
    ...
    return SRV_LED_STATE_BREATHING;
}
...
static void led_fsm_on_entry(fsm_t* ctx, fsm_state_t state)
{
    ...
    switch (state) {
    case SRV_LED_STATE_ON:
        led_phys_write(handle, LED_PWM_MAX);
        break;
    case SRV_LED_STATE_OFF:
        led_phys_write(handle, 0);
        break;
    case SRV_LED_STATE_BLINK_CODE:
        handle->current_led_blink_code_counts = 0;
        handle->blink_code_phase_last = handle->blink_code_phase;
        handle->blink_code_phase = SRV_LED_BLINK_PHASE_INTERVAL;
...
static void led_process_cmds(srv_led_handle_t* handle)
{
    ...
    while (msg_fifo_pop(&handle->cmd_fifo, &cmd)) {
        if (cmd.led_set_state != SRV_LED_STATE_NONE) {
            fsm_goto(&handle->fsm, cmd.led_set_state);
        }

        if (cmd.led_set_state == SRV_LED_STATE_BLINK_CODE) {
            if (handle->pending_blink_update && fsm_current_state(&handle->fsm) == SRV_LED_STATE_BLINK_CODE) {
                if (handle->blink_sw_on) {
            ...
            if (fsm_current_state(&handle->fsm) == SRV_LED_STATE_BLINK_CODE) {
                handle->current_led_blink_code_counts = 0;
                handle->blink_code_phase = SRV_LED_BLINK_PHASE_INTERVAL;
                handle->interval_start_time = s_led_get_time ? s_led_get_time() : 0;
                handle->blink_sw_on = false;
        ...
        if (cmd.led_set_state == SRV_LED_STATE_BREATHING || cmd.led_breath_cycle_ms > 0) {
...
void srv_led_deinit(void)
{
    ...
    {
        srv_led_handle_t* h = clist_entry(pos, srv_led_handle_t, node);
        clist_del(pos);
...
srv_led_handle_t* srv_led_get_instance(const char* name)
{
...
clist_head_t* srv_led_get_head(void)
{
    return s_led_initialized ? &s_led_head : NULL;
}

led_error_t srv_led_register_static(const srv_led_config_t* config,
    srv_led_handle_t* instance)
{
    ...
    if (srv_led_get_instance(config->name))
        return LED_ERROR_ALREADY_EXIST;

    memset(instance, 0, sizeof(srv_led_handle_t));
    memcpy(&instance->config, config, sizeof(srv_led_config_t));
    ...
    handlers[SRV_LED_STATE_NONE] = led_fsm_static_handler;
    handlers[SRV_LED_STATE_OFF] = led_fsm_static_handler;
    handlers[SRV_LED_STATE_ON] = led_fsm_static_handler;
    handlers[SRV_LED_STATE_BLINK_CODE] = led_fsm_blink_handler;
    handlers[SRV_LED_STATE_BREATHING] = led_fsm_breathing_handler;
    ...
    /* 每实例独立的命令队列（存储在句柄内，支持多 LED 注册互不干扰） */
    msg_fifo_init(&instance->cmd_fifo, instance->cmd_buffer,
        sizeof(instance->cmd_buffer), sizeof(srv_led_cmd_t));
...
led_error_t srv_led_unregister(const char* name)
{
    if (name == NULL || !s_led_initialized)
        return LED_ERROR_INVALID_PARAM;

    srv_led_handle_t *h, *tmp;
...
void srv_led_set_state(srv_led_handle_t* instance, srv_led_state_t state)
{
    if (instance == NULL)
        return;
...
led_error_t srv_led_set_blink_interval(srv_led_handle_t* instance,
    const srv_led_cmd_t* cmd)
{
    if (instance == NULL || cmd == NULL)
        return LED_ERROR_INVALID_PARAM;

    bool changed = (instance->current_cmd.led_set_state != SRV_LED_STATE_BLINK_CODE)
    ...
    if (fsm_current_state(&instance->fsm) == SRV_LED_STATE_BLINK_CODE) {
        if (instance->blink_sw_on) {
            memcpy(&instance->current_cmd, cmd, sizeof(srv_led_cmd_t));
            instance->current_cmd.led_set_state = SRV_LED_STATE_BLINK_CODE;
        ...
        instance->blink_code_phase = SRV_LED_BLINK_PHASE_INTERVAL;
...
srv_led_blink_phase_t srv_led_get_blink_phase(srv_led_handle_t* instance)
{
    return instance ? (srv_led_blink_phase_t)instance->blink_code_phase
                    : SRV_LED_BLINK_PHASE_INTERVAL;
}

void srv_led_set_callbacks(srv_led_handle_t* instance, led_state_change_cb_t state_cb,
...
void srv_led_task_refresh(void)
{
