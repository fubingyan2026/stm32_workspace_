 ...
 * @attention
 *
 * 自 stm32_g474_boot/service/boot/boot_fsm.h 移植。
...
/* Block 帧间隔超时：容忍上位机 USB 逐帧节奏（Windows 下 ep_in 阻塞读可达 ~15ms/帧）。
 * 100ms 在 171 帧/块（经典 CAN 8B）下过紧，会被慢主机偶发 >100ms 间隔误杀；
...
struct boot_fsm_context {
    ...
    /* 块重试统计 */
    uint8_t  block_retry_count;     /**< 当前块连续校验失败次数（成功后清零，用于重试日志） */
    ...
    /* IDLE 心跳（beacon） */
    uint32_t last_beacon_tick;      /**< 上次发送 beacon 的 tick 值 */
    ...
    /* REBOOT 延迟复位 */
    bool reset_requested;          /**< 请求系统复位（ACK 发出后再复位，由 boot_task 消费） */
 ...
 * @brief 取出延迟复位请求（一次性）
 *
 * REBOOT 处理不立即复位：FSM 置 reset_requested 后由 boot_task 在 ACK 真正发出
 * 后再执行系统复位（否则 ACK 未上总线即被复位打断，上位机收不到）。
 *
 * @param ctx 状态机上下文指针
 * @return true 表示需要执行系统复位
 */
