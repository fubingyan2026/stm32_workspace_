...
can_task_init()     → CAN (MUST precede power_task — registers read_data callback) + buzzer PWM init (`drv_buzzer_init`)
...
ws2812_task_init()  → WS2812B strip (SPI1/SPI3) init & comet demo (auto comet off — RGB by CAN)
...
The middleware offers more, but this app uses: `sw_timer` (all tasks), `fsm` (in `srv_signal` and `srv_pwr_ctrl`), `msg_fifo` (in `srv_signal`, `srv_adc`, and `srv_can_mst` TX queue), `kfifo`/`clist` (underpinning those), and `log`. `daemon` is no longer used by this app (the `srv_device_monitor` online-detection service was removed). `event` is **not** wired up in this project.
