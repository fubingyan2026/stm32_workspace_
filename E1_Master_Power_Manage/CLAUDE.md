...
In `can_rx_callback`: `0x001`+len=7 → `srv_can_mst_process_rx`; every other ID falls through to `srv_can_slv_process_rx` then `srv_can_dual_process_rx` (each service ignores frames it doesn't own).
