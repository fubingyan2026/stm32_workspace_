...
The middleware offers more, but this app uses: `sw_timer` (all tasks), `fsm` (in `srv_led` and `srv_pwr_ctrl`), `msg_fifo` (in `srv_led`, `srv_adc`, and `srv_can_mst` TX queue), `kfifo`/`clist` (underpinning those), and `log`. `daemon` and `event` are **not** wired up in this project.
...
- **`srv_can_mst`**: `can_task`'s `power_task_read_status` packs board status (power faults, E-STOP, fan faults, dual-battery snapshot) into `srv_can_mst_data_t`, queued via `msg_fifo`, one frame per task tick with a 100ms periodic `srv_can_mst_request(0x00)` (status frame only; battery frames on-demand from host).
- **`srv_can_slv`**: sends 0x002 control frame, waits for ACK (50ms timeout with retry). `can_task_set_slave_ctrl()` sets the HSD/reserved control bits from other tasks.
