...
from .protocol import (
    ...
    CMD_VERIFY, CMD_REBOOT, CMD_ACK, CMD_NACK, CMD_BEACON,
    BOOT_REQUEST_CAN_ID,
    ...
    build_verify, build_reboot, build_cancel, build_boot_request,
    parse_header, parse_ack, parse_nack, parse_nack_block_index, parse_beacon,
...
# 发 0x003 后等待复位进 boot + 首帧 beacon 的最大时间。
# 直接发 0x003 而非先探测 beacon：0x003 对已在 Boot 的设备无害（Boot 忽略），
# 省掉整个探测窗口，App→Boot 由 ~3.5s 降到 ~1s。
# 1s 覆盖：App 复位(~10ms) + Boot 初始化(~100ms) + 首帧 beacon(≤500ms) ≈ ~0.6s。
# 若 App 侧 ring_storage 偶发 GC（擦除 128KB 扇区 ~1-2s）导致复位延迟，可能误超时——遇此可调大。
TRIGGER_WAIT_TIMEOUT = 1.0
# DATA 途中轮询间隔（帧）：ep_in 阻塞读在 Windows 下实际耗时 ~15ms/次，逐帧轮询会把下载拖到
# ~50s（171 帧/块 × 85 块）。每 16 帧探测一次：错误仍能在 ≤16 帧内被拦截重同步，
# 同时下载降到 ~15s。板端块看门狗已放宽到 1000ms，15ms 轮询间隙无压力。
DATA_POLL_INTERVAL = 16
...
class FlashConfig:
    def __init__(self, *, fw_path: str, bitrate: int = 1_000_000,
                 hw_compat_id: int = 2, version: int = 1,
                 fd_mode: bool = False, data_bitrate: int = 2_000_000,
                 max_frame_size: int = 8, serial: Optional[str] = None,
                 can_id: int = 0x701, boot_request_can_id: int = BOOT_REQUEST_CAN_ID,
        ...
        self.boot_request_can_id = boot_request_can_id  # 进 boot 触发帧 CAN ID（App 态）
        self.frame_interval_us = frame_interval_us  # 每帧 DATA 发送间隔（0=不限速）


def _delay_us(interval_us: int):
    """帧间隔延时：亚毫秒用忙等保证精度，毫秒级用 sleep 省 CPU。

    Windows 的 time.sleep 粒度 ~1-16ms，无法满足亚毫秒限速，故 <1000µs 走忙等。
    """
    if interval_us <= 0:
        return
    if interval_us >= 1000:
        ...
        while (time.perf_counter() - t0) * 1_000_000 < interval_us:
            pass
...
class FlashWorker(QObject):
    ...
    def _wait_block(self, expected_cmd: int,
                    expected_idx: Optional[int] = None) -> tuple[bool, Optional[CANFrame]]:
        """等待块级 ACK/NACK，校验应答命令字（可选块号），过期帧忽略重轮询。

        返回 (is_ack, frame_or_None)。若不校验命令字，块间残留的过期帧（如上一块
        DATA_START ACK / 前次重试的 NACK）会被误当成当前块的应答，导致主机提前推进、
        板端停在旧块，引发重同步风暴与校验失败。
        """
        while True:
            ...
            if cmd == CMD_ACK:
                if acked != expected_cmd:
                    self._log_d("BLOCK", f"忽略过期 ACK: acked=0x{acked:02X} (期望 0x{expected_cmd:02X})")
                    continue
                if expected_idx is not None:
                    idx = int.from_bytes(data[3:5], "big") if len(data) >= 5 else -1
                    if idx != expected_idx:
                        self._log_d("BLOCK", f"忽略过期块号 ACK: idx={idx} (期望 {expected_idx})")
                        continue
                return True, resp
            if cmd == CMD_NACK:
                if acked != expected_cmd:
                    self._log_d("BLOCK", f"忽略过期 NACK: acked=0x{acked:02X} (期望 0x{expected_cmd:02X})")
                    continue
                return False, resp
            # 非 ACK/NACK（如 beacon）：忽略重轮询
            continue
    ...
    def _send_trigger(self):
        """发送进 boot 触发帧（App 态专用，CAN ID 可配，默认 0x003）。

        载荷固定 1 字节（DLC=1，App 端按 dlc>=1 且 data[0]==0x01 判定）；
        切勿 ljust(8) 补足——那会使 DLC=8，App 无法识别。
        """
        frame = CANFrame(
            can_id=self._config.boot_request_can_id,
            data=build_boot_request(),
            extended=False,
            fd=False,
            brs=False,
        )
    ...
    def _wait_for_beacon(self, timeout: float, expected_hw_id: int) -> bool:
        """等待 Boot 心跳（beacon）帧，确认设备已处于升级模式。

        返回 true 表示在超时内收到 hw_id 匹配的 beacon。
        """
        end_time = time.monotonic() + timeout
        while time.monotonic() < end_time:
            if self._cancel_requested:
            ...
            if frame is None:
                continue
            if frame.can_id != self._config.can_id_resp:
                continue
            # 0x702 帧即节点存活（beacon 或残留 ACK），刷新失联判定基准
            self._last_rx_monotonic = time.monotonic()
            hw_id = parse_beacon(frame.data)
            if hw_id is not None and hw_id == expected_hw_id:
                return True
        return False
    ...
    def _run_flash(self, config: FlashConfig):
        ...
        # ── Phase 0: 触发进 boot（直接发 0x003）──
        # 设备在 App → 0x003 触发复位进 Boot；已在 Boot → Boot 忽略 0x003（无害）。
        # 等 beacon 确认（含 hw_id 核对，防刷错板），比「先探测 2.5s 再触发」快得多。
        self._log_i("PHASE", f"发送 0x003 触发进入升级模式 (hw_id=0x{config.hw_compat_id:04X})")
        self._send_trigger()
        if not self._wait_for_beacon(TRIGGER_WAIT_TIMEOUT, config.hw_compat_id):
            raise TimeoutError(
                "未检测到 Boot 心跳：可能 App 未处理 0x003，或设备异常；请复位后重试")
        self._log_i("PHASE", "已进入 Boot，开始升级握手")
        ...
        while block_idx < total_blocks:
            ...
            if self._cancel_requested:
                raise RuntimeError("用户取消")
            ack, resp = self._wait_block(CMD_DATA_START, block_idx)
            if not ack:
            ...
            for seq_idx in range(frames_per_block - 1):
                ...
                # 流控：按配置的帧间隔限速（0=不限速）
                _delay_us(config.frame_interval_us)
                # 每 N 帧短超时轮询一次：拦截途中重同步 NACK，避免逐帧阻塞读拖慢传输
                if (seq_idx % DATA_POLL_INTERVAL) == (DATA_POLL_INTERVAL - 1):
            ...
            if n_code == STATUS_BLOCK_CHECKSUM:
                ...
                self._log_w("BLOCK", f"块 {block_idx+1} 校验失败 (第 {attempt}/{MAX_RETRIES} 次)，重发该块...")
                continue
            if resp is None:
