...
对应 E1_Master_Power_Manage docs/boot_upgrade.md §4（自 stm32_g474_boot 移植）。
所有帧基于标准 CAN 11-bit ID；E1 为经典 CAN，帧长固定 8 字节。
"""
...
# 进 boot 触发命令帧（App 态专用，走独立 CAN ID 0x003，非 boot 协议 0x701）
BOOT_REQUEST_CAN_ID = 0x003
...
CMD_DATA_START = 0x07
CMD_DATA_END = 0x08
CMD_BEACON = 0x09  # Boot 心跳（板卡→上位机，IDLE 态每 1s 一帧，E1 扩展）
CMD_ACK = 0x10
CMD_NACK = 0x11
...
def build_boot_request() -> bytes:
    """构造进 boot 触发帧（App 态专用，发往 CAN ID 0x003）。[0x01]"""
    return bytes([0x01])
...
def parse_beacon(data: bytes) -> Optional[int]:
    """解析 Boot 心跳（beacon）帧，返回 hw_id；非 beacon 帧返回 None。

    格式：[0x09][hw_id_H][hw_id_L][填充]（0x702，8 字节经典 CAN）。
    """
    if len(data) < 3 or data[0] != CMD_BEACON:
        return None
