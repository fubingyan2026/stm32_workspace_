 ...
 * @brief   CAN 引导协议帧编解码（纯数据变换，无状态）
 * @attention
 *
 * 自 stm32_g474_boot/service/boot/boot_transport.h 移植。
 * 本模块负责升级协议帧与 drv_can_msg_t 之间的转换。
 * 所有函数为纯数据变换，可重入，不维护任何内部状态。
 * STM32F407 为经典 bxCAN（仅 8 字节帧），协议帧长集合已裁为 {8}。
 */
...
#define BOOT_FRAME_SIZES_COUNT       1U     /**< 离散长度数量 */
...
typedef enum {
    ...
    BOOT_CMD_BEACON    = 0x09U, /**< Boot 心跳（板卡 → 上位机，IDLE 态每 1s 一帧，E1 扩展） */
 ...
 * @brief 解析 START 帧
 * @param msg CAN 消息
 * @param fw_size 输出：固件总大小 (uint32, Byte 1-4)
 * @param hw_id 输出：硬件兼容 ID (uint16, Byte 5-6)
 * @param max_frame_size 输出：单帧物理长度 (uint8, Byte 7)
 * @return true 表示帧格式有效
 */
...
/**
 * @brief 构造 Boot 心跳（beacon）帧
 * @param msg 输出：CAN 消息
 * @param hw_id 本机硬件兼容 ID（写入 Byte 1-2，大端序，供上位机核对目标板）
 * @note  IDLE 态每 1s 发送一帧（0x702，8 字节经典 CAN）：
 *        [0x09][hw_id_H][hw_id_L][0][0][0][0][0]
 */
void boot_transport_build_beacon(drv_can_msg_t* msg, uint16_t hw_id);
