...
/** 支持的经典 CAN 帧长度（STM32F407 为 bxCAN，仅 8 字节） */
static const uint8_t s_supported_frame_sizes[] = {
    BOOT_FRAME_SIZE_CLASSIC_CAN
};
...
{
    /* METADATA 帧：cmd(1) + checksum(4) + version(2)，最少 7 字节（无 seq 字节） */
    if (msg->dlc < 7U) {
        return false;
    }
...
void boot_transport_build_beacon(drv_can_msg_t* msg, uint16_t hw_id)
{
    ...
    msg->id = BOOT_CAN_ID_NODE_TO_HOST;
    msg->is_extended = false;
    msg->data[0] = BOOT_CMD_BEACON;
    msg->data[1] = (uint8_t)(hw_id >> 8);
    msg->data[2] = (uint8_t)(hw_id & 0xFFU);
    /* 其余填充 0 */
    msg->dlc = 8U;
}
