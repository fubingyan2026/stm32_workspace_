...
flowchart TD
    ...
    B --> C{"目标扇区为空?"}
    C -- 否 --> D["擦除目标扇区<br/>（回绕，清理旧数据）"]
    C -- 是 --> E["读最新帧帧头"]
    D --> E
    E --> F["分块复制最新帧到目标扇区<br/>（每块对齐到 write_gran）"]
    F --> G["切换活动扇区<br/>active_index = next<br/>（旧扇区保留历史，懒擦除）"]
    G --> H["GC 完成"]
```
...
**懒擦除**：旧活动扇区在切换后**不立即擦除**，其历史帧保留，直到回绕到该扇区时才被擦除。因此任意时刻 Flash 上保留最多 N × 扇区容量的历史帧（上图中标记为 "Empty" 的扇区在稳态下实际保存历史数据），`load_version` 可回溯更多版本。
...
- **懒擦除**：旧活动扇区不立即擦除，保留历史帧直到回绕，整个区域全部参与存储（历史深度 N × 扇区容量），`load_version` 可回溯更多版本
