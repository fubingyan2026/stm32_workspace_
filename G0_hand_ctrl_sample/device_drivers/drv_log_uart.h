 ...
 * @version V1.0.0
 * @date    2026-08-12
 * @brief   USART2 控制台串口设备驱动（DMA 输出 + DMA circular + IDLE 中断 → kfifo 接收）
 * @attention
 *
 * 参考 E1_Master_Power_Manage 工程的 drv_log_uart 设计：
 *   - USART2 为 G0 手套唯一的 UART，独立管理（日志输出 TX + 控制台命令 RX）
 *   - RX：DMA circular + IDLE 中断，按 DMA 计数器同步进 kfifo（环形缓冲），
 *     消费方在主循环通过 drv_log_uart_rx_read() 轮询读取
 *   - TX：非阻塞 DMA 发送
 *
 * F1 差异：无 UART_CLEAR_OREF/NEF/FEF/PEF 与 UART_RXDATA_FLUSH_REQUEST 宏，
...
/**
 * @brief 初始化 USART2 控制台驱动（内部状态，直接使用 CubeMX 的 huart2）
 * @return 操作结果错误码
 */
