 ...
 * @version V1.0.0
 * @date    2026-09-03
 * @brief   ADC 设备驱动实现（DMA + 通道路由表，E1_MASTER_POWER_CTU）
 */
...
/**
 * @brief 内置通道路由表（与 Core/Src/adc.c MX_ADC1_Init 的 Rank 顺序严格对应）
 *
 * DMA 缓冲区索引 = Rank - 1
 *
 * ADC1 (INST_1, 7ch): PC0/PC1/PC2/PC3/PC4 + TEMPSENSOR + VREFINT
 *   Rank1..7 = NTC1(IN10), NTC2(IN11), VIN_DC-DC(IN12), VIN(IN13),
 *              CD4051B(IN14, 预留), TEMPSENSOR, VREFINT
 */
static const drv_adc_route_t s_routes[DRV_ADC_CH_MAX] = {
    [DRV_ADC_CH_NTC1]      = { DRV_ADC_INST_1, 0 }, /**< PC0, ADC1_IN10, Rank1 */
    [DRV_ADC_CH_NTC2]      = { DRV_ADC_INST_1, 1 }, /**< PC1, ADC1_IN11, Rank2 */
    [DRV_ADC_CH_VIN_DC_DC] = { DRV_ADC_INST_1, 2 }, /**< PC2, ADC1_IN12, Rank3 */
    [DRV_ADC_CH_VIN]       = { DRV_ADC_INST_1, 3 }, /**< PC3, ADC1_IN13, Rank4 */
    [DRV_ADC_CH_CD4051B]   = { DRV_ADC_INST_1, 4 }, /**< PC4, ADC1_IN14, Rank5 (预留) */
    [DRV_ADC_CH_TEMPSENSOR]= { DRV_ADC_INST_1, 5 }, /**< 内部温度传感器, Rank6 */
    [DRV_ADC_CH_VREFINT]   = { DRV_ADC_INST_1, 6 }, /**< 内部参考电压, Rank7 */
};
...
void drv_adc_init(void)
{
    ...
    DRV_ADC_LOG_I("ADC 初始化完成: 实例数=%u, ADC1 通道数=%u",
        (unsigned)DRV_ADC_INST_NUM, (unsigned)s_ctx[0].channel_count);
}
