/**
 * @file    srv_can.h
 * @brief   旧 CAN FD 上位机控制协议服务 — 控制帧解析 + 反馈/状态帧打包
 *
 * 苇熠伺服执行器测试协议（经典 CAN 2.0A, 1 Mbps，设备地址寻址）已独立为
 * srv_ht_temp_test.c/h，本模块仅保留旧 CAN FD 上位机协议：
 *
 ...
 * @param msg CAN 报文指针
 * @note  先路由给 srv_ht_temp_test_on_rx() 消费测试协议帧，剩余帧按旧协议缓存，
 *        不直接调用电机 API（ISR 安全）
 */
