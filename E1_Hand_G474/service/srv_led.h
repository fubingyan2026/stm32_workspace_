 ...
 */
typedef enum __attribute__((packed)) {
    SRV_LED_BLINK_PHASE_BLINKING = 0, /**< 闪烁中：按 cycle_ms 切换亮灭 */
    SRV_LED_BLINK_PHASE_INTERVAL, /**< 间隔中：按 wait_ms 保持熄灭 */
} srv_led_blink_phase_t;

/**
 * @brief LED 句柄前向声明
 */
typedef struct led_handle srv_led_handle_t;
 ...
 */
typedef void (*led_state_change_cb_t)(srv_led_handle_t* instance,
    srv_led_state_t new_state, void* user_data);
 ...
 * @param user_data 用户数据
 */
typedef void (*led_blink_phase_cb_t)(srv_led_handle_t* instance,
    srv_led_blink_phase_t phase, void* user_data);
 ...
 * @param user_data 用户数据
 */
typedef void (*led_edge_cb_t)(srv_led_handle_t* instance, bool rising,
    void* user_data);
...
#define SRV_LED_CMD_BUF_SIZE (128U)
...
typedef struct {
    ...
    srv_led_state_t init_state; /**< 初始状态 */
 ...
 */
typedef struct {
    srv_led_state_t led_set_state; /**< 目标状态 */
...
struct led_handle {
    ...
    srv_led_cmd_t current_cmd; /**< 当前命令参数 */
    uint32_t last_toggle_time; /**< 上次翻转时间戳 */
    uint32_t interval_start_time; /**< 间隔阶段起始时间戳 */

    uint16_t current_led_blink_code_counts; /**< 当前闪烁计数 */
    srv_led_blink_phase_t blink_code_phase; /**< 当前闪烁阶段 */
    srv_led_blink_phase_t blink_code_phase_last; /**< 上次闪烁阶段 */
    ...
    msg_fifo_t cmd_fifo; /**< 异步命令队列实例（每实例独立，支持多 LED 注册） */
    uint8_t cmd_buffer[SRV_LED_CMD_BUF_SIZE]; /**< 命令队列缓冲区（2 的幂字节） */
