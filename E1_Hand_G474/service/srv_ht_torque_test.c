...
void srv_ht_torque_test_step(void)
{
    ...
    for (uint32_t i = 0; i < s_motor_cnt; i++) {
        ...
        if (s_dir > 0) {
            if (s_motor_pos_iq[i] >= SRV_HT_TORQUE_TEST_POS_LIMIT_IQ - SRV_HT_TORQUE_TEST_REACH_IQ) {
                s_dir = -1;
                s_flipped = true;
            }
        } else {
            if (s_motor_pos_iq[i] <= SRV_HT_TORQUE_TEST_REACH_IQ) {
                s_dir = 1;
                s_flipped = true;
            }
        }
    ...
    if ((now - s_last_status_ms) >= SRV_HT_TORQUE_TEST_STATUS_LOG_MS) {
        ...
        for (uint32_t i = 0; i < s_motor_cnt; i++) {
            ...
            SRV_HT_TORQUE_TEST_LOG_W("状态: 电机 0x%02X pos=%ld dir=%d 下发=%dRPM 使能=%d 模式=%u 实际=%ldRPM 报警=0x%lX 在线年龄=%lu ms",
