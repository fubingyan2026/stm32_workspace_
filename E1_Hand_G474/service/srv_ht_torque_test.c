/**
 * @file    srv_ht_torque_test.c
 * @brief   苇熠(HT) 伺服执行器 CAN 控制协议服务实现 — 速度模式多圈往复耐久测试
 *
 ...
 * 默认设备地址（SRV_HT_TORQUE_TEST_DEFAULT_MOTOR_ADDR）继续控制。
 *
 * RX 侧：srv_ht_torque_test_on_rx() 只消费测试协议帧（扫描应答/位置/报警/电压/
 * 在线心跳），返回 true 表示已处理；旧 CAN FD 上位机控制帧（0x100）由
 * srv_can_on_rx() 解析。
 *
...
#define SRV_HT_TORQUE_TEST_LOG_ENABLE 1

#if SRV_HT_TORQUE_TEST_LOG_ENABLE
#define SRV_HT_TORQUE_TEST_LOG_I(...) //LOG_I("srv_ht_torque_test", __VA_ARGS__)
#define SRV_HT_TORQUE_TEST_LOG_W(...) LOG_W("torque_test", __VA_ARGS__)
#define SRV_HT_TORQUE_TEST_LOG_E(...) LOG_E("torque_test", __VA_ARGS__)
#else
#define SRV_HT_TORQUE_TEST_LOG_I(...) ((void)0)
#define SRV_HT_TORQUE_TEST_LOG_W(...) ((void)0)
#define SRV_HT_TORQUE_TEST_LOG_E(...) ((void)0)
#endif
/* 模块测试开关 ----------------------------------------------------------------*/

/** @brief 电机测试模式：1=上电自动启动（扫描 + 速度模式多圈往复循环）；0=需手动调用 srv_ht_torque_test_start() */
#define SRV_HT_TORQUE_TEST_AUTO_START 1
...
#define SRV_HT_TORQUE_TEST_POS_LIMIT_DEG (6 * 360)
/**
 * @brief 往复半幅换算为 IQ24 位置值（单位 R）：15R = 0x00F00000
 * @note  IQ = deg/360 × 2^24（文档 §4.2：位置 IQ24 值即为实际转数，满量程 ±127R）
 */
const int32_t SRV_HT_TORQUE_TEST_POS_LIMIT_IQ = ((SRV_HT_TORQUE_TEST_POS_LIMIT_DEG / 360) << 24);

/** @brief 到达端点判定阈值 (deg)：|位置-端点| ≤ 该角度视为到达端点（随后反向） */
#define SRV_HT_TORQUE_TEST_REACH_DEG 3
...
#define SRV_HT_TORQUE_TEST_NORESP_PERIOD_MS 10000U
/**
 * @brief 耐久运行时长 (ms)：电机持续在线累计满 30 天自动停止并失能。
 *        30 天 = 30×24×3600×1000 ms = 2,592,000,000 ms（uint32 范围内）；掉线期间不计时
 */
#define SRV_HT_TORQUE_TEST_DURATION_MS 2592000000U
...
#define SRV_HT_TORQUE_TEST_FLIP_TIMEOUT_MS 60000U
/** @brief 周期状态诊断日志 (ms)：打印各电机 位置/方向/转速/报警/在线年龄。
 *        60s 一次避免刷屏；确认耐久运行正常后可置大或关闭 */
#define SRV_HT_TORQUE_TEST_STATUS_LOG_MS 60000U
/** @brief 1=记录并打印所有收到的 CAN 帧（确认 0x06 位置应答真实格式）；0=关闭 */
#define SRV_HT_TORQUE_TEST_LOG_RAW_RX_ENABLE 0
/** @brief 原始帧环形缓冲深度 */
#define SRV_HT_TORQUE_TEST_RAW_RX_DEPTH 16U
...
#define SRV_HT_TORQUE_TEST_VOLTAGE_OFFSET_MS 250U
#endif

/* --- 总线电机扫描参数 --- */

/** @brief 扫描地址范围（含）：协议推荐 0x01~0x3F，0x00 为广播且无返回 */
#define SRV_HT_TORQUE_TEST_SCAN_ADDR_MIN 0x01U
#define SRV_HT_TORQUE_TEST_SCAN_ADDR_MAX 0x3FU
/** @brief 每地址探测间隔 (ms)：留出电机握手应答时间 */
#define SRV_HT_TORQUE_TEST_SCAN_PROBE_PERIOD_MS 20U
/** @brief 总线重扫周期 (ms)：电机晚于控制板上电（热插拔）时错过启动扫描，
 *        周期重扫握手探测 0x01~0x3F 发现新电机并接管使能 */
#define SRV_HT_TORQUE_TEST_RESCAN_PERIOD_MS 3000U
/** @brief 最多可同时控制的电机数 */
#define SRV_HT_TORQUE_TEST_MAX_MOTORS 63U
/** @brief 未扫描到电机时使用的默认设备地址（CAN-ID 低 8 位） */
#define SRV_HT_TORQUE_TEST_DEFAULT_MOTOR_ADDR 0x21U

/* --- 苇熠协议指令符（docs/苇熠电机can协议文档.md §6） --- */

#define SRV_HT_TORQUE_TEST_CMD_HANDSHAKE 0x00U /**< 握手：发送 1 字节，电机返回 [0x00][状态] */
#define SRV_HT_TORQUE_TEST_CMD_ENABLE 0x2AU /**< 使能/失能：参数 0x01 使能，0x00 失能 */
#define SRV_HT_TORQUE_TEST_CMD_MODE 0x07U /**< 设置模式 */
#define SRV_HT_TORQUE_TEST_CMD_SPEED 0x09U /**< 设置速度值：IQ24，归一化 × 速度满量程（正=正转，负=反转，0=停止） */
#define SRV_HT_TORQUE_TEST_CMD_POS_READ 0x06U /**< 读取当前位置值：返回 [0x06][4B 大端 IQ24]（5B） */
#define SRV_HT_TORQUE_TEST_CMD_ALARM 0xFFU /**< 查询报警：发送 1 字节，实测返回 [0xFF][4B 大端报警码]（5B） */
#define SRV_HT_TORQUE_TEST_CMD_VOLTAGE 0x87U /**< 读取供电电压：返回 [0x87][V高][V低]，×100 单位 V（仅使能时刷新） */
#define SRV_HT_TORQUE_TEST_CMD_ENABLE_STATE 0x2BU /**< 查询使能/失能状态：返回 [0x2B][0x01/0x00]（2B） */
#define SRV_HT_TORQUE_TEST_CMD_MODE_QUERY 0x55U /**< 查询当前模式：返回 [0x55][模式值]（2B，读取指令1） */
...
#define SRV_HT_TORQUE_TEST_ENABLE_KEEPALIVE_MS 2000U
#define SRV_HT_TORQUE_TEST_ENABLE_KEEPALIVE_OFFSET_MS 11U
/** @brief 使能保持补发相位：0=查询0x2B，1=补发使能，2=补发模式，3=复查（每相位占一个 keepalive 周期，
 *        保证任一 tick 对单电机至多发 1 帧，规避 FDCAN TX FIFO 深度 3 突爆丢帧） */
static uint8_t s_keepalive_phase[SRV_HT_TORQUE_TEST_MAX_MOTORS];

/** @brief 未使能补发日志锁存：同一次失能期间只打印一次（电机确认使能后清除） */
static bool s_enable_log_latch[SRV_HT_TORQUE_TEST_MAX_MOTORS];
...
typedef struct {
    uint32_t mask; /**< 错误码位 */
    const char* name; /**< 含义 */
} srv_ht_torque_test_alarm_desc_t;

/** @brief 报警码 → 含义映射表（协议文档 §8；0xFF 应答为 [0xFF][4B 大端]，支持全部 24 位码） */
static const srv_ht_torque_test_alarm_desc_t s_alarm_map[] = {
    { 0x00000001U, "过压" },
    { 0x00000002U, "欠压" },
    { 0x00000004U, "堵转" },
    { 0x00000008U, "过热" },
...
/** @brief 报警码描述表项数 */
#define SRV_HT_TORQUE_TEST_ALARM_NUM (sizeof(s_alarm_map) / sizeof(s_alarm_map[0]))

#if SRV_HT_TORQUE_TEST_LOG_RAW_RX_ENABLE
/** @brief 原始接收帧记录（调试用，ISR 写入主循环打印） */
typedef struct {
    uint32_t id; /**< CAN ID */
    uint8_t dlc; /**< 数据长度 */
    uint8_t data[8]; /**< 前 8 字节 */
} srv_ht_torque_test_raw_rx_t;
#endif
...
static uint8_t s_motor_ids[SRV_HT_TORQUE_TEST_MAX_MOTORS];
...
static int32_t s_motor_pos_iq[SRV_HT_TORQUE_TEST_MAX_MOTORS];

/** @brief 新位置应答待处理标志（ISR 置位，主循环清零后做端点反向判定） */
static bool s_motor_pos_pending[SRV_HT_TORQUE_TEST_MAX_MOTORS];

/** @brief 每电机往复中心 (IQ24，单位 R)：各电机初始化（首次读到位置）时的位置作为自身 0 点，
 *        目标在 各自中心 ± POS_LIMIT 两端点间交替（多电机各以自身初始化位置摆动） */
static int32_t s_center_iq[SRV_HT_TORQUE_TEST_MAX_MOTORS];

/** @brief 每电机往复中心是否已锁存（首个位置应答后锁存；恢复在线后重新锁存） */
static bool s_center_latched[SRV_HT_TORQUE_TEST_MAX_MOTORS];

/** @brief 每电机最新使能状态（0x2B 应答更新，0=失能，1=使能，ISR 写） */
static bool s_motor_enabled[SRV_HT_TORQUE_TEST_MAX_MOTORS];

/** @brief 每电机使能状态是否已确认（收到过 0x2B 应答；未确认时不补发使能） */
static bool s_motor_enable_known[SRV_HT_TORQUE_TEST_MAX_MOTORS];

/** @brief 每电机最新模式（0x55 应答更新，ISR 写） */
static uint8_t s_motor_mode[SRV_HT_TORQUE_TEST_MAX_MOTORS];

/** @brief 每电机最新实际速度 IQ24（0x05 应答更新，×6000 RPM，ISR 写） */
static int32_t s_motor_speed_iq[SRV_HT_TORQUE_TEST_MAX_MOTORS];
...
static uint32_t s_motor_alarm[SRV_HT_TORQUE_TEST_MAX_MOTORS];

/** @brief 新报警应答待打印标志（ISR 置位，主循环清零后打印） */
static bool s_alarm_pending[SRV_HT_TORQUE_TEST_MAX_MOTORS];

/** @brief 上一轮已处理的报警码（主循环维护，用于状态变化检测） */
static uint32_t s_alarm_last[SRV_HT_TORQUE_TEST_MAX_MOTORS];

/** @brief 每电机最后收到任何帧的时间 (millis)，用于掉线检测（ISR 更新） */
static uint32_t s_motor_last_seen_ms[SRV_HT_TORQUE_TEST_MAX_MOTORS];

/** @brief 电机无响应告警锁存（收到任何帧后清除，避免重复刷屏） */
static bool s_motor_nresp_latch[SRV_HT_TORQUE_TEST_MAX_MOTORS];

/** @brief 恢复在线事件待打印标志（ISR 置位，主循环清零打印） */
static bool s_online_evt_pending[SRV_HT_TORQUE_TEST_MAX_MOTORS];
...
static srv_ht_torque_test_raw_rx_t s_raw_rx[SRV_HT_TORQUE_TEST_RAW_RX_DEPTH];
...
static uint16_t s_motor_volt[SRV_HT_TORQUE_TEST_MAX_MOTORS];

/** @brief 新电压应答待打印标志（ISR 置位，主循环清零） */
static bool s_volt_pending[SRV_HT_TORQUE_TEST_MAX_MOTORS];
...
static uint32_t srv_ht_torque_test_find_idx(uint8_t addr);
static bool srv_ht_torque_test_all_online(void);
static void srv_ht_torque_test_scan_record(uint8_t addr);
static void srv_ht_torque_test_scan_log_new(void);
static void srv_ht_torque_test_scan_done(void);
static void srv_ht_torque_test_rescan_done(uint32_t now);
...
static void srv_ht_torque_test_query_alarm_all(void);
static void srv_ht_torque_test_send_query_enable_state(uint8_t addr);
static void srv_ht_torque_test_send_query_mode(uint8_t addr);
static void srv_ht_torque_test_send_query_speed(uint8_t addr);
static void srv_ht_torque_test_alarm_print(uint8_t addr, uint32_t code);
#if SRV_HT_TORQUE_TEST_LOG_RAW_RX_ENABLE
static void srv_ht_torque_test_raw_rx_push(const drv_can_msg_t* msg);
static void srv_ht_torque_test_raw_rx_drain(void);
#endif
#if SRV_HT_TORQUE_TEST_VOLTAGE_QUERY_ENABLE
static void srv_ht_torque_test_send_query_voltage(uint8_t addr);
static void srv_ht_torque_test_query_voltage_all(void);
static void srv_ht_torque_test_voltage_print(uint8_t addr, uint16_t volt);
#endif
...
{
#if SRV_HT_TORQUE_TEST_AUTO_START
    srv_ht_torque_test_start(); /* 测试模式：先扫描总线电机 ID，再下发控制命令 */
#endif
}
...
void srv_ht_torque_test_start(void)
{
    ...
    /* 阶段 1：扫描总线电机 ID */
    s_scanning = true;
    s_scan_addr = SRV_HT_TORQUE_TEST_SCAN_ADDR_MIN;
    s_motor_cnt = 0;
    s_scan_log_cnt = 0;
    s_scan_last_probe_ms = s_start_ms - SRV_HT_TORQUE_TEST_SCAN_PROBE_PERIOD_MS;
    s_rescanning = false;
    ...
    SRV_HT_TORQUE_TEST_LOG_I("耐久测试启动：正在扫描总线电机 ID 0x%02X~0x%02X（每 %u ms 探测一个地址）",
        SRV_HT_TORQUE_TEST_SCAN_ADDR_MIN, SRV_HT_TORQUE_TEST_SCAN_ADDR_MAX,
        (unsigned)SRV_HT_TORQUE_TEST_SCAN_PROBE_PERIOD_MS);
}
...
void srv_ht_torque_test_stop(void)
{
    ...
    s_ramp_from_rpm = 0;
    s_ramp_start_ms = 0;
    srv_ht_torque_test_cmd_speed_all(0); /* 速度 0 停下 */
    srv_ht_torque_test_cmd_enable_all(false); /* 失能 */
    SRV_HT_TORQUE_TEST_LOG_I("耐久测试停止：已向 %u 台电机发送速度 0 + 失能", (unsigned)s_motor_cnt);
}
...
void srv_ht_torque_test_step(void)
{
    ...
    if (!s_scanning && srv_ht_torque_test_all_online()) {
        s_online_ms += (uint32_t)(now - s_online_last_ms);
    }
    s_online_last_ms = now;

    if (s_online_ms >= SRV_HT_TORQUE_TEST_DURATION_MS) {
        SRV_HT_TORQUE_TEST_LOG_I("电机持续在线累计 %lu ms 已到（30 天），耐久测试自动停止",
            (unsigned long)s_online_ms);
        srv_ht_torque_test_stop();
        return;
    }

    /* 阶段 1：扫描总线电机 ID（逐地址发送握手 0x00，按应答 CAN-ID 记录） */
    if (s_scanning) {
        srv_ht_torque_test_scan_log_new(); /* 打印新探测到的电机 */

        if ((now - s_scan_last_probe_ms) >= SRV_HT_TORQUE_TEST_SCAN_PROBE_PERIOD_MS) {
            s_scan_last_probe_ms = now;
            srv_ht_torque_test_send_handshake(s_scan_addr);
            s_scan_addr++;
            if (s_scan_addr > SRV_HT_TORQUE_TEST_SCAN_ADDR_MAX) {
                srv_ht_torque_test_scan_done();
            }
        }
        return;
    }
...
#if SRV_HT_TORQUE_TEST_LOG_RAW_RX_ENABLE
    srv_ht_torque_test_raw_rx_drain(); /* 调试：打印原始接收帧（确认 0x06 位置应答格式） */
#endif
    ...
    if (!s_rescanning && ((s_motor_cnt == 0U) || s_fallback_active) &&
        ((now - s_last_rescan_ms) >= SRV_HT_TORQUE_TEST_RESCAN_PERIOD_MS)) {
        s_rescanning = true;
        s_rescan_addr = SRV_HT_TORQUE_TEST_SCAN_ADDR_MIN;
        s_rescan_last_probe_ms = now - SRV_HT_TORQUE_TEST_SCAN_PROBE_PERIOD_MS;
        s_rescan_start_cnt = s_motor_cnt;
        if (s_fallback_active) {
            s_motor_cnt = 0; /* 清空回退占位，由重扫重新发现真实电机 */
            s_fallback_active = false;
            s_rescan_start_cnt = 0;
            SRV_HT_TORQUE_TEST_LOG_W("热插拔重扫：清除回退占位地址，重新探测总线");
        }
    }
    if (s_rescanning) {
        if ((now - s_rescan_last_probe_ms) >= SRV_HT_TORQUE_TEST_SCAN_PROBE_PERIOD_MS) {
            s_rescan_last_probe_ms = now;
            srv_ht_torque_test_send_handshake(s_rescan_addr);
            s_rescan_addr++;
            if (s_rescan_addr > SRV_HT_TORQUE_TEST_SCAN_ADDR_MAX) {
                s_rescanning = false;
                s_last_rescan_ms = now;
                srv_ht_torque_test_rescan_done(now);
            }
        }
        return; /* 重扫期间只发握手探测 */
    }
    ...
    for (uint32_t i = 0; i < s_motor_cnt; i++) {
        if (s_alarm_pending[i]) {
            ...
            if (code != prev) { /* 报警状态变化 */
                if (code != 0U) {
                    srv_ht_torque_test_alarm_print(s_motor_ids[i], code); /* 报警出现/变化 */
                } else {
                    SRV_HT_TORQUE_TEST_LOG_W("电机 0x%02X 报警已消除，恢复正常", (unsigned)s_motor_ids[i]);
                }
                s_alarm_last[i] = code;
            }
            /* 报警状态未变化：不打印 */
    ...
    for (uint32_t i = 0; i < s_motor_cnt; i++) {
        if (s_volt_pending[i]) {
            ...
            srv_ht_torque_test_voltage_print(s_motor_ids[i], s_motor_volt[i]);
        }
    }
#endif
    ...
    if ((now - s_last_pos_poll_ms) >= SRV_HT_TORQUE_TEST_POS_POLL_PERIOD_MS) {
        s_last_pos_poll_ms = now;
        srv_ht_torque_test_query_position_all();
    }
    ...
    for (uint32_t i = 0; i < s_motor_cnt; i++) {
        if (!s_center_latched[i] && s_motor_pos_pending[i]) {
            ...
            SRV_HT_TORQUE_TEST_LOG_I("已锁存往复中心：电机 0x%02X 初始位置 %ld 为 0 点，区间 [%ld, %ld]",
                (unsigned)s_motor_ids[i], (long)s_center_iq[i],
                (long)((int64_t)s_center_iq[i] - (int64_t)SRV_HT_TORQUE_TEST_POS_LIMIT_IQ),
                (long)((int64_t)s_center_iq[i] + (int64_t)SRV_HT_TORQUE_TEST_POS_LIMIT_IQ));
        }
    }
    ...
    for (uint32_t i = 0; i < s_motor_cnt; i++) {
        ...
        const int64_t hi = center + (int64_t)SRV_HT_TORQUE_TEST_POS_LIMIT_IQ;
        const int64_t lo = center - (int64_t)SRV_HT_TORQUE_TEST_POS_LIMIT_IQ;
        if (s_dir > 0) {
            if ((int64_t)s_motor_pos_iq[i] >= (hi - (int64_t)SRV_HT_TORQUE_TEST_REACH_IQ)) {
                s_dir = -1;
                s_flipped = true;
            }
        } else {
            if ((int64_t)s_motor_pos_iq[i] <= (lo + (int64_t)SRV_HT_TORQUE_TEST_REACH_IQ)) {
                s_dir = 1;
                s_flipped = true;
            }
        }
    ...
    if ((now - s_last_flip_ms) >= SRV_HT_TORQUE_TEST_FLIP_TIMEOUT_MS) {
        s_dir = (s_dir > 0) ? -1 : 1;
        s_last_flip_ms = now;
        SRV_HT_TORQUE_TEST_LOG_W("端点反向超时（%lu ms 未翻转），强制反向",
            (unsigned long)SRV_HT_TORQUE_TEST_FLIP_TIMEOUT_MS);
    }

    /* 速度平滑斜坡：方向变化时在 RAMP_MS 内线性爬升/下降，消除加减速突变/卡顿 */
    if ((now - s_last_cmd_ms) >= SRV_HT_TORQUE_TEST_CMD_PERIOD_MS) {
        s_last_cmd_ms = now;

        const int16_t target_rpm = (s_dir > 0) ? SRV_HT_TORQUE_TEST_SPEED_RPM
                                               : -SRV_HT_TORQUE_TEST_SPEED_RPM;
        ...
        if (elapsed >= SRV_HT_TORQUE_TEST_RAMP_MS) {
            ...
            send_rpm = (int16_t)((int32_t)s_ramp_from_rpm
                + ((int32_t)(target_rpm - s_ramp_from_rpm) * (int32_t)elapsed)
                    / (int32_t)SRV_HT_TORQUE_TEST_RAMP_MS);
        }
        s_speed_rpm = send_rpm;
        srv_ht_torque_test_cmd_speed_all(send_rpm);
    }

    /* 周期查询电机报警（1s 一次，0xFF 主动查询，应答由 on_rx 记录） */
    if ((now - s_last_alarm_ms) >= SRV_HT_TORQUE_TEST_ALARM_PERIOD_MS) {
        s_last_alarm_ms = now;
        srv_ht_torque_test_query_alarm_all();
    }
    ...
    if ((now - s_last_keepalive_ms) >= SRV_HT_TORQUE_TEST_ENABLE_KEEPALIVE_MS) {
        s_last_keepalive_ms = now;
        for (uint32_t i = 0; i < s_motor_cnt; i++) {
            switch (s_keepalive_phase[i]) {
            case 0: /* 查询使能状态 */
                srv_ht_torque_test_send_query_enable_state(s_motor_ids[i]);
                break;
            case 1: /* 已确认未使能：补发使能 */
                if (s_motor_enable_known[i] && !s_motor_enabled[i]) {
                    ...
                    if (!s_enable_log_latch[i]) {
                        ...
                        SRV_HT_TORQUE_TEST_LOG_W("电机 0x%02X 未使能，已补发使能", (unsigned)s_motor_ids[i]);
                    }
                }
                break;
            case 2: /* 补发速度模式 */
                if (s_motor_enable_known[i] && !s_motor_enabled[i]) {
                    ...
                    if (!s_enable_log_latch[i]) {
                        s_enable_log_latch[i] = true;
                        SRV_HT_TORQUE_TEST_LOG_W("电机 0x%02X 未使能，已补发速度模式", (unsigned)s_motor_ids[i]);
                    }
                }
                break;
            case 3: /* 复查使能状态 */
    ...
    if ((now - s_last_volt_ms) >= SRV_HT_TORQUE_TEST_VOLTAGE_PERIOD_MS) {
        s_last_volt_ms = now;
        srv_ht_torque_test_query_voltage_all();
    }
#endif
    ...
    if ((now - s_last_status_ms) >= SRV_HT_TORQUE_TEST_STATUS_LOG_MS) {
        s_last_status_ms = now;
        for (uint32_t i = 0; i < s_motor_cnt; i++) {
            if (s_status_q_phase) {
                srv_ht_torque_test_send_query_mode(s_motor_ids[i]);
            } else {
                srv_ht_torque_test_send_query_speed(s_motor_ids[i]);
            }
            const int32_t act_rpm = (int32_t)(((int64_t)s_motor_speed_iq[i] * SRV_HT_TORQUE_TEST_SPEED_FULL_SCALE) >> 24);
            SRV_HT_TORQUE_TEST_LOG_W("状态: 电机 0x%02X pos=%ld ctr=%ld dir=%d 下发=%dRPM 使能=%d 模式=%u 实际=%ldRPM 报警=0x%lX 在线年龄=%lu ms",
    ...
    for (uint32_t i = 0; i < s_motor_cnt; i++) {
        if (!s_motor_nresp_latch[i] && ((now - s_motor_last_seen_ms[i]) >= SRV_HT_TORQUE_TEST_NORESP_PERIOD_MS)) {
            s_motor_nresp_latch[i] = true;
            SRV_HT_TORQUE_TEST_LOG_W("电机 0x%02X 长时间无响应（掉线或断电）", (unsigned)s_motor_ids[i]);
        }
    }
...
bool srv_ht_torque_test_on_rx(const drv_can_msg_t* msg)
{
...
#if SRV_HT_TORQUE_TEST_LOG_RAW_RX_ENABLE
    srv_ht_torque_test_raw_rx_push(msg); /* 调试：记录原始接收帧 */
#endif

    /* 扫描阶段或热插拔重扫阶段：应答帧 CAN-ID 即电机地址，记录（越界由 record 过滤） */
    if (s_scanning || s_rescanning) {
        srv_ht_torque_test_scan_record((uint8_t)(msg->id & 0xFFU));
        return true;
    }
    ...
    const uint32_t idx = srv_ht_torque_test_find_idx(addr);
    if (idx == SRV_HT_TORQUE_TEST_MAX_MOTORS)
        return false;
    ...
    if ((msg->dlc >= 5U) && (msg->data[0] == SRV_HT_TORQUE_TEST_CMD_POS_READ)) {
    ...
    if ((msg->dlc >= 5U) && (msg->data[0] == SRV_HT_TORQUE_TEST_CMD_ALARM)) {
    ...
    if ((msg->dlc >= 2U) && (msg->data[0] == SRV_HT_TORQUE_TEST_CMD_ENABLE_STATE)) {
    ...
    if ((msg->dlc >= 2U) && (msg->data[0] == SRV_HT_TORQUE_TEST_CMD_MODE_QUERY)) {
        s_motor_mode[idx] = msg->data[1];
        return true;
    }

    /* 实际速度应答：data[0]=0x05，data[1..4]=IQ24（×6000 RPM，读取指令3，5B） */
    if ((msg->dlc >= 5U) && (msg->data[0] == SRV_HT_TORQUE_TEST_CMD_SPEED_READ)) {
        const uint32_t u = ((uint32_t)msg->data[1] << 24) | ((uint32_t)msg->data[2] << 16) | ((uint32_t)msg->data[3] << 8) | (uint32_t)msg->data[4];
    ...
    if ((msg->dlc >= 3U) && (msg->data[0] == SRV_HT_TORQUE_TEST_CMD_VOLTAGE)) {
 ...
 * @return 索引；未找到返回 SRV_HT_TORQUE_TEST_MAX_MOTORS（哨兵值）
 */
static uint32_t srv_ht_torque_test_find_idx(uint8_t addr)
{
    for (uint32_t i = 0; i < s_motor_cnt; i++) {
    ...
    return SRV_HT_TORQUE_TEST_MAX_MOTORS;
}
...
static void srv_ht_torque_test_scan_record(uint8_t addr)
{
    if ((addr < SRV_HT_TORQUE_TEST_SCAN_ADDR_MIN) || (addr > SRV_HT_TORQUE_TEST_SCAN_ADDR_MAX))
        return;
    ...
    if (s_motor_cnt < SRV_HT_TORQUE_TEST_MAX_MOTORS) {
...
static void srv_ht_torque_test_scan_log_new(void)
{
    while (s_scan_log_cnt < s_motor_cnt) {
        SRV_HT_TORQUE_TEST_LOG_I("  探测到电机：CAN ID = 0x%02X", (unsigned)s_motor_ids[s_scan_log_cnt]);
        s_scan_log_cnt++;
    }
}
...
static void srv_ht_torque_test_scan_done(void)
{
    ...
    s_last_alarm_ms = s_ctrl_start_ms - SRV_HT_TORQUE_TEST_ALARM_OFFSET_MS;
    /* 使能保持补发同样错开：起始偏移 11ms（非 20/100ms 整数倍对齐），
       与位置/速度/报警帧分时，保证 keepalive tick 帧数不超 TX FIFO 深度 3 */
    s_last_keepalive_ms = s_ctrl_start_ms + SRV_HT_TORQUE_TEST_ENABLE_KEEPALIVE_OFFSET_MS - SRV_HT_TORQUE_TEST_ENABLE_KEEPALIVE_MS;
#if SRV_HT_TORQUE_TEST_VOLTAGE_QUERY_ENABLE
    s_last_volt_ms = s_ctrl_start_ms - SRV_HT_TORQUE_TEST_VOLTAGE_OFFSET_MS;
#endif

    /* 未扫描到电机：回退到默认设备地址继续测试（总线可能因电机未应答握手而探测失败）；
     * 置 s_fallback_active 标记：热插拔重扫前若仍无应答则清空占位，由重扫重新发现真实电机 */
    if (s_motor_cnt == 0U) {
        SRV_HT_TORQUE_TEST_LOG_W("扫描完成：未检测到电机（0x%02X~0x%02X），回退到默认 CAN ID 0x%02X",
            SRV_HT_TORQUE_TEST_SCAN_ADDR_MIN, SRV_HT_TORQUE_TEST_SCAN_ADDR_MAX, SRV_HT_TORQUE_TEST_DEFAULT_MOTOR_ADDR);
        s_motor_ids[0] = SRV_HT_TORQUE_TEST_DEFAULT_MOTOR_ADDR;
        s_motor_cnt = 1;
        s_fallback_active = true;
    } else {
        SRV_HT_TORQUE_TEST_LOG_I("扫描完成：检测到 %u 台电机", (unsigned)s_motor_cnt);
        s_fallback_active = false;
    }

    for (uint8_t i = 0; i < s_motor_cnt; i++) {
        SRV_HT_TORQUE_TEST_LOG_I("  电机[%u] CAN ID = 0x%02X", (unsigned)i, (unsigned)s_motor_ids[i]);
    }
    ...
    SRV_HT_TORQUE_TEST_LOG_I("开始多圈往复：初始化中心 ±%d 圈（±%d°），速度模式连续旋转，累计在线 %lu ms（30 天）后停止",
...
static void srv_ht_torque_test_rescan_done(uint32_t now)
{
    if (s_motor_cnt > s_rescan_start_cnt) {
        SRV_HT_TORQUE_TEST_LOG_I("热插拔重扫完成：检测到 %u 台电机", (unsigned)(s_motor_cnt - s_rescan_start_cnt));
    }
...
static void srv_ht_torque_test_send_handshake(uint8_t addr)
{
    ...
    tx.data[0] = SRV_HT_TORQUE_TEST_CMD_HANDSHAKE;
...
static void srv_ht_torque_test_send_enable(uint8_t addr, bool enable)
{
    ...
    tx.data[0] = SRV_HT_TORQUE_TEST_CMD_ENABLE;
...
static void srv_ht_torque_test_set_speed_mode(uint8_t addr)
{
    ...
    tx.data[0] = SRV_HT_TORQUE_TEST_CMD_MODE;
    tx.data[1] = SRV_HT_TORQUE_TEST_MODE_SPEED;
    drv_can_send(DRV_CAN_CH_1, &tx);
}
...
static void srv_ht_torque_test_send_speed(uint8_t addr, int16_t rpm)
{
    ...
    const int32_t iq = (int32_t)(((int64_t)rpm << 24) / SRV_HT_TORQUE_TEST_SPEED_FULL_SCALE);
    ...
    tx.data[0] = SRV_HT_TORQUE_TEST_CMD_SPEED;
...
static void srv_ht_torque_test_send_query_position(uint8_t addr)
{
    ...
    tx.data[0] = SRV_HT_TORQUE_TEST_CMD_POS_READ;
    drv_can_send(DRV_CAN_CH_1, &tx);
}
...
{
    for (uint8_t i = 0; i < s_motor_cnt; i++) {
        srv_ht_torque_test_send_enable(s_motor_ids[i], enable);
    }
}
...
{
    for (uint8_t i = 0; i < s_motor_cnt; i++) {
        srv_ht_torque_test_send_speed(s_motor_ids[i], rpm);
    }
}
...
{
    for (uint8_t i = 0; i < s_motor_cnt; i++) {
        srv_ht_torque_test_send_query_position(s_motor_ids[i]);
    }
}
...
static void srv_ht_torque_test_send_query_alarm(uint8_t addr)
{
    ...
    tx.data[0] = SRV_HT_TORQUE_TEST_CMD_ALARM;
    drv_can_send(DRV_CAN_CH_1, &tx);
}
...
static void srv_ht_torque_test_send_query_enable_state(uint8_t addr)
{
    ...
    tx.data[0] = SRV_HT_TORQUE_TEST_CMD_ENABLE_STATE;
    drv_can_send(DRV_CAN_CH_1, &tx);
}
...
static void srv_ht_torque_test_send_query_mode(uint8_t addr)
{
    ...
    tx.data[0] = SRV_HT_TORQUE_TEST_CMD_MODE_QUERY;
    drv_can_send(DRV_CAN_CH_1, &tx);
}
...
static void srv_ht_torque_test_send_query_speed(uint8_t addr)
{
    ...
    tx.data[0] = SRV_HT_TORQUE_TEST_CMD_SPEED_READ;
    drv_can_send(DRV_CAN_CH_1, &tx);
}
...
{
    for (uint8_t i = 0; i < s_motor_cnt; i++) {
        srv_ht_torque_test_send_query_alarm(s_motor_ids[i]);
    }
}
...
{
    if (code == 0U) {
        SRV_HT_TORQUE_TEST_LOG_I("电机 0x%02X 报警查询：无报警", (unsigned)addr);
        return;
    }

    SRV_HT_TORQUE_TEST_LOG_W("电机 0x%02X 报警：0x%08X", (unsigned)addr, (unsigned)code);
    for (uint32_t i = 0; i < SRV_HT_TORQUE_TEST_ALARM_NUM; i++) {
        if ((code & s_alarm_map[i].mask) != 0U) {
            SRV_HT_TORQUE_TEST_LOG_W("  - %s", s_alarm_map[i].name);
        }
    }
}
...
static void srv_ht_torque_test_send_query_voltage(uint8_t addr)
{
    ...
    tx.data[0] = SRV_HT_TORQUE_TEST_CMD_VOLTAGE;
    drv_can_send(DRV_CAN_CH_1, &tx);
}
...
{
    for (uint8_t i = 0; i < s_motor_cnt; i++) {
        srv_ht_torque_test_send_query_voltage(s_motor_ids[i]);
    }
}
...
static void srv_ht_torque_test_voltage_print(uint8_t addr, uint16_t volt)
{
    SRV_HT_TORQUE_TEST_LOG_I("电机 0x%02X 供电电压：%u.%02u V",
...
static void srv_ht_torque_test_raw_rx_push(const drv_can_msg_t* msg)
{
    ...
    srv_ht_torque_test_raw_rx_t* slot = &s_raw_rx[head];
    slot->id = msg->id;
    slot->dlc = msg->dlc;
    ...
    head = (uint8_t)((head + 1U) % SRV_HT_TORQUE_TEST_RAW_RX_DEPTH);
    if (head == s_raw_rx_tail) {
        /* 环形满：丢弃最旧，保留最新 */
        s_raw_rx_tail = (uint8_t)((s_raw_rx_tail + 1U) % SRV_HT_TORQUE_TEST_RAW_RX_DEPTH);
    }
    s_raw_rx_head = head;
}
...
static void srv_ht_torque_test_raw_rx_drain(void)
{
    while (s_raw_rx_tail != s_raw_rx_head) {
        const srv_ht_torque_test_raw_rx_t* slot = &s_raw_rx[s_raw_rx_tail];
        s_raw_rx_tail = (uint8_t)((s_raw_rx_tail + 1U) % SRV_HT_TORQUE_TEST_RAW_RX_DEPTH);
        SRV_HT_TORQUE_TEST_LOG_W("RX id=0x%03lX dlc=%u %02X %02X %02X %02X %02X %02X %02X %02X",
