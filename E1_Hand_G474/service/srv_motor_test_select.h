 ...
 *   - SRV_MOTOR_TEST_TZ_TEMP   = 3 良志(ODrive) 速度模式 24H 耐久测试（srv_tz_temp_test）
 *
 ...
 *   - SRV_MOTOR_TEST_TZ_TEMP_CAN2 = 2 良志(ODrive) 速度模式耐久 CAN2 版（srv_tz_temp_test 实例）
 */
...
typedef enum {
    ...
    SRV_MOTOR_TEST_SEL_TZ_TEMP = 3, /* 良志(ODrive) 速度模式 24H 耐久（srv_tz_temp_test） */
...
typedef enum {
    ...
    SRV_MOTOR_TEST_SEL_TZ_TEMP_CAN2 = 2, /* 良志(ODrive) 速度模式耐久 CAN2 版（srv_tz_temp_test 实例） */
...
#define SRV_MOTOR_TEST_TZ_TEMP 3

//can2的选项电机
#define SRV_MOTOR_TEST_HT_CAN2 0
#define SRV_MOTOR_TEST_PA430 1
#define SRV_MOTOR_TEST_TZ_TEMP_CAN2 2

/* --- 当前选择（命令行 -DSRV_MOTOR_TEST_SELECT=SRV_MOTOR_TEST_HT_TORQUE 等可覆盖） --- */

/**
 * @brief 当前激活的 CAN1 测试模块（默认良志 TZ_TEMP 速度模式 24H 耐久；如需切换，
 *        临时改本默认值后重新编译即可，勿依赖命令行 -D，CMake 不透传该宏）
 */
#ifndef SRV_MOTOR_TEST_SELECT
#define SRV_MOTOR_TEST_SELECT SRV_MOTOR_TEST_TZ_TEMP
#endif

/**
 * @brief 当前激活的 CAN2 测试模块（默认良志 TZ_TEMP_CAN2 速度模式耐久；如需切换，
 *        临时改本默认值后重新编译即可，勿依赖命令行 -D，CMake 不透传该宏）
 */
#ifndef SRV_MOTOR_TEST_SELECT_CAN2
#define SRV_MOTOR_TEST_SELECT_CAN2 SRV_MOTOR_TEST_TZ_TEMP_CAN2
#endif
...
#define SRV_MOTOR_TEST_IS_TZ_TEMP (SRV_MOTOR_TEST_SELECT == SRV_MOTOR_TEST_TZ_TEMP)
...
#define SRV_MOTOR_TEST_IS_TZ_TEMP_CAN2 (SRV_MOTOR_TEST_SELECT_CAN2 == SRV_MOTOR_TEST_TZ_TEMP_CAN2)
