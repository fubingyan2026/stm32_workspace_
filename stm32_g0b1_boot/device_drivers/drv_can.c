 ...
 *
 * G0 适配：仅 FDCAN2（PB12/PB13），drv_can_init() 绑定 &hfdcan2。
 */

/* Includes ------------------------------------------------------------------*/
#include "drv_can.h"

#include "fdcan.h" /* extern hfdcan2：drv_can_init(void) 无参初始化使用 */
#include "main.h"
...
{
    /* G0 仅 FDCAN2 (PB12/PB13)；与 public_layer boot_task.c 的 drv_can_init()
     * 调用契约一致：内部使用 CubeMX 全局句柄 hfdcan2 初始化 DRV_CAN_CH_1。 */
    return drv_can_init_channel(DRV_CAN_CH_1, &hfdcan2);
}
