 ...
 * @attention
 *
 * STM32G0B1CBTx 使用 FDCAN2 外设 (hfdcan2, PB12/PB13)。
 ...
 */
typedef enum {
    DRV_CAN_CH_1 = 0, /**< FDCAN2 — PB12(RX) / PB13(TX)，G0 主用通道 */
    DRV_CAN_CH_2,     /**< 保留（G0 无第二路 CAN） */
    DRV_CAN_CH_NUM,   /**< 通道总数 */
} drv_can_channel_t;
...
/**
 * @brief 初始化 FDCAN2（无参，boot/升级栈使用）
 *
 * 与 public_layer boot_task.c 的 drv_can_init() 调用契约一致：
 * 内部使用 CubeMX 全局句柄 hfdcan2 初始化 DRV_CAN_CH_1。
 */
