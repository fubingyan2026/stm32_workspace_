...
本模块是协议实现的**唯一来源**（纯 Python，不依赖 Qt / pyserial）：
上位机 GUI（``app``）与 Linux SDK（``ctu_sdk``）共用此处的编解码。
...
#: 公开 API：``from ctu_sdk.protocol import *`` 与 GUI 侧再导出均依赖此表
__all__ = [
    # 设备
    "DEV_ADDR_MASTER", "DEV_ADDR_SLAVER", "DEV_NAMES",
    # 命令 / 错误码
    "CMD_READ_STATUS", "CMD_READ_VOLT", "CMD_READ_TEMP", "CMD_CTRL",
    "CMD_RESET_LATCH", "CMD_UPGRADE", "CMD_READ_INFO",
    "CMD_REPLY_FLAG", "CMD_ERR", "CMD_NAME", "ERR_TEXT",
    # 帧编解码
    "crc8", "build_frame", "FrameParser",
    "parse_addr", "parse_cmd", "parse_payload",
    "frame_hex", "describe_frame",
    # 小端解包
    "unpack_i16_le", "unpack_u16_le", "unpack_u32_le",
    # MASTER 数据段
    "MST_STATUS_KEYS", "decode_mst_status", "decode_mst_volt",
    "decode_mst_temp",
    # SLAVER 数据段
    "SLV_STATUS_KEYS", "decode_slv_status", "decode_slv_volt",
    "decode_slv_temp",
    # 构建下行帧
    "build_read_status", "build_read_volt", "build_read_temp",
    "build_mst_ctrl", "build_slv_ctrl", "build_reset_latch",
    "build_read_info", "build_upgrade",
    # 固件信息（0x07）
    "INFO_FLAG_META_VALID", "INFO_FLAG_UPGRADE_REQ", "INFO_FLAG_UPGRADE_DONE",
    "decode_info", "info_flags_text",
]
