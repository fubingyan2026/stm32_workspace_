 ...
 * @version V1.0.0
 * @date    2026-09-03
 * @brief   PWM 设备驱动（内置通道路由表，多通道通用，E1_SLAVER_POWER_CTU）
 * @attention
 *
 * 硬件配置（路由表）内置在 drv_pwm.c 中，上层无需传参。
 * drv_pwm_init() 自动初始化全部已注册 PWM 通道（幂等，可被多个驱动重复调用）。
 *
 * ## 通道分配（共用 TIM4，定时器时钟 72MHz，默认 20kHz）
 * - DRV_PWM_CH_LED : 板载状态指示灯 (PB6 → TIM4_CH1)
 * - DRV_PWM_CH_FILL: 补光灯 DIM (PB8 → TIM4_CH3，PT4115，20kHz)
 *
 * ## 用法
 * @code
 *   drv_pwm_init();
 *   drv_pwm_set_duty(DRV_PWM_CH_FILL, 500);
 * @endcode
 *
 * 占空比使用千分比 0~1000（0.1% 精度）。PWM1 模式下 duty=0 输出恒低（关），
 * duty=1000 输出恒高（开）。
 *
 * @note 两个逻辑通道共享同一 TIM4，修改频率按「组」生效（所有通道同时变）。
 *       LED 与补光因此固定工作在同一频率（默认 20kHz），亮度仅靠占空比调节。
 */
 ...
 */
typedef enum {
    DRV_PWM_CH_LED = 0,      /**< LED_PWM — 板载状态指示灯 (PB6, TIM4_CH1) */
    DRV_PWM_CH_FILL,         /**< FILL_LED_PWM — 补光灯 DIM (PB8, TIM4_CH3) */
    DRV_PWM_CH_MAX,
} drv_pwm_channel_t;
