 ...
 * @version V1.0.0
 * @date    2026-09-03
 * @brief   ADC 设备驱动（单 ADC1 DMA + 内置通道路由表，E1_SLAVER_POWER_CTU）
 * @attention
 *
 ...
 *
 * ## ADC 实例分配
 * - ADC1: PC0/PC1/PC2/PC3 (IN10~IN13) + 内部通道 (TEMPSENSOR/VREFINT)
 *   Rank1..6 = AUX, MOTOR, LSD1, LSD2, TEMPSENSOR, VREFINT
 *
 * ## 用法
 * @code
 *   drv_adc_init();
 ...
 *   drv_adc_trigger_all();
 *   uint32_t raw = drv_adc_read_raw(DRV_ADC_CH_AUX);
 * @endcode
 */
 ...
 */
typedef enum {
    DRV_ADC_CH_AUX = 0,         /**< AUX_POWER_ADC — 副电源输入电压 (PC0, ADC1_IN10, Rank1) */
    DRV_ADC_CH_MOTOR,           /**< MOTOR_POWER_ADC — 电机电源输入电压 (PC1, ADC1_IN11, Rank2) */
    DRV_ADC_CH_LSD1,            /**< LSD1_ADC — 低边开关 1 输出节点 (PC2, ADC1_IN12, Rank3) */
    DRV_ADC_CH_LSD2,            /**< LSD2_ADC — 低边开关 2 输出节点 (PC3, ADC1_IN13, Rank4) */
    DRV_ADC_CH_TEMPSENSOR,      /**< MCU 内部温度传感器 (ADC1_IN16, Rank5) */
    DRV_ADC_CH_VREFINT,         /**< 内部参考电压 ~1.20V (ADC1_IN17, Rank6) */
