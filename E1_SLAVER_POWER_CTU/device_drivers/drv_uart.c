 ...
 *  @note  必须容纳最长 485 帧：升级 DATA 帧 = 248(数据) + 5(负载头) + 5(信封) = 258B。
 *         原 256 小于单帧，DMA 回卷会静默丢弃未读数据（kfifo_move_in 里 out=in-size），
 *         主循环被周期任务/日志拖慢时表现为升级块应答超时。取 2048（与 E1_CTU_BOOT 一致）：
 *         单帧不跨回卷，且留约 178ms@115200 的主循环抖动余量。 */
