/**
 * @file    drv_power.c
 * @author  maximillian
 * @version V1.0.0
 * @date    2026-09-03
 * @brief   电源输出使能控制驱动实现（E1_SLAVER_POWER_CTU）
 */

/* Includes ------------------------------------------------------------------*/
#include "drv_power.h"

#include "log.h"
#include "main.h"

/* 模块日志开关 ----------------------------------------------------------------*/

/** @brief 本文件日志开关：置 0 屏蔽本文件全部打印 */
#define DRV_POWER_LOG_ENABLE 1

#if DRV_POWER_LOG_ENABLE
#define DRV_POWER_LOG_E(...) LOG_E("drv_power", __VA_ARGS__)
#define DRV_POWER_LOG_W(...) LOG_W("drv_power", __VA_ARGS__)
#define DRV_POWER_LOG_I(...) LOG_I("drv_power", __VA_ARGS__)
#define DRV_POWER_LOG_D(...) LOG_D("drv_power", __VA_ARGS__)
#else
#define DRV_POWER_LOG_E(...) ((void)0)
#define DRV_POWER_LOG_W(...) ((void)0)
#define DRV_POWER_LOG_I(...) ((void)0)
#define DRV_POWER_LOG_D(...) ((void)0)
#endif

/* Private types -------------------------------------------------------------*/

typedef struct {
    GPIO_TypeDef* port;
    uint16_t      pin;
    bool          active_low; /**< true 表示低电平有效（拉低使能） */
    const char*   name;
} drv_power_rail_pin_t;

/* Private constants ---------------------------------------------------------*/

/**
 * @brief 电源输出引脚配置表
 *
 * 引脚宏定义来自 Core/Inc/main.h，每路对应 CubeMX 已配置/待配置的 GPIO 输出。
 * （LSD1_IN/LSD2_IN 在 .ioc 中已改为 GPIO_Output，初始化时统一按推挽输出重配。）
 */
static const drv_power_rail_pin_t s_pins[DRV_POWER_RAIL_NUM] = {
    [DRV_POWER_RAIL_DC24V]  = { DC_DC_EN_GPIO_Port,   DC_DC_EN_Pin,   false, "DC_DC_EN(24V)" },
    [DRV_POWER_RAIL_ISO12V] = { ISO_EN_12V_GPIO_Port, ISO_EN_12V_Pin, false, "ISO_EN_12V" },
    [DRV_POWER_RAIL_LSD1]   = { LSD1_IN_GPIO_Port,    LSD1_IN_Pin,    false, "LSD1_IN" },
    [DRV_POWER_RAIL_LSD2]   = { LSD2_IN_GPIO_Port,    LSD2_IN_Pin,    false, "LSD2_IN" },
};

/* Private variables ---------------------------------------------------------*/

static bool s_initialized;

/* Exported functions --------------------------------------------------------*/

void drv_power_init(void)
{
    if (s_initialized) {
        drv_power_deinit();
    }

    GPIO_InitTypeDef gpio = {
        .Mode = GPIO_MODE_OUTPUT_PP,
        .Pull = GPIO_NOPULL,
        .Speed = GPIO_SPEED_FREQ_LOW,
    };

    for (uint32_t i = 0; i < DRV_POWER_RAIL_NUM; i++) {
        if (s_pins[i].port && s_pins[i].pin) {
            /* 统一重配为推挽输出，初始拉低（关） */
            gpio.Pin = s_pins[i].pin;
            HAL_GPIO_Init(s_pins[i].port, &gpio);
            HAL_GPIO_WritePin(s_pins[i].port, s_pins[i].pin,
                s_pins[i].active_low ? GPIO_PIN_SET : GPIO_PIN_RESET);
        }
    }

    s_initialized = true;
    DRV_POWER_LOG_I("电源输出初始化完成 (%u 路全部关闭)", (unsigned)DRV_POWER_RAIL_NUM);
}

void drv_power_deinit(void)
{
    if (!s_initialized) {
        return;
    }

    for (uint32_t i = 0; i < DRV_POWER_RAIL_NUM; i++) {
        if (s_pins[i].port && s_pins[i].pin) {
            HAL_GPIO_WritePin(s_pins[i].port, s_pins[i].pin,
                s_pins[i].active_low ? GPIO_PIN_SET : GPIO_PIN_RESET);
        }
    }

    s_initialized = false;
    DRV_POWER_LOG_I("电源输出反初始化完成");
}

void drv_power_set(drv_power_rail_t rail, bool on)
{
    if (!s_initialized || rail >= DRV_POWER_RAIL_NUM) {
        DRV_POWER_LOG_W("电源输出设置被忽略: rail=%u 越界或未初始化", (unsigned)rail);
        return;
    }

    if (s_pins[rail].port && s_pins[rail].pin) {
        GPIO_PinState level =
            (on ^ s_pins[rail].active_low) ? GPIO_PIN_SET : GPIO_PIN_RESET;
        HAL_GPIO_WritePin(s_pins[rail].port, s_pins[rail].pin, level);
    }

    DRV_POWER_LOG_D("电源输出 %s -> %s", drv_power_rail_name(rail), on ? "开启" : "关闭");
}

void drv_power_toggle(drv_power_rail_t rail)
{
    if (!s_initialized || rail >= DRV_POWER_RAIL_NUM) {
        DRV_POWER_LOG_W("电源输出设置被忽略: rail=%u 越界或未初始化", (unsigned)rail);
        return;
    }

    if (s_pins[rail].port && s_pins[rail].pin) {
        HAL_GPIO_TogglePin(s_pins[rail].port, s_pins[rail].pin);
    }
}

const char* drv_power_rail_name(drv_power_rail_t rail)
{
    if (rail >= DRV_POWER_RAIL_NUM) {
        return "INVALID";
    }
    return s_pins[rail].name ? s_pins[rail].name : "UNNAMED";
}
