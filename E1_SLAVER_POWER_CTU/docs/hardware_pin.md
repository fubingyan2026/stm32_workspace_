...
* **Pin 20 (PA4) — `12V_ISO_EN`**：推挽输出（GPIO Output），控制隔离 12V 模块（URB2412S-6WR3）的 CTRL 引脚（高电平开启，低电平关闭）。
* **Pin 21 (PA5) — `12V_ISO_PGOOD`**：输入（GPIO Input），经比较器 LM193 与光耦隔离反馈，检测 12V_ISO 是否在正常窗口范围（11.4V ~ 12.6V）（高电平正常，低电平异常）。
