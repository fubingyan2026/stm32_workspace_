...
#define SRV_COM_SLV_LOG_ENABLE 1

#if SRV_COM_SLV_LOG_ENABLE
...
#define SRV_COM_SLV_LOG_I(...) ((void)0)
#define SRV_COM_SLV_LOG_D(...) ((void)0)
#endif
...
#define SRV_COM_SLV_BUS_MAX_FRAME_LEN (258U)

/** @brief parser 输入 kfifo 缓冲（必须为 2 的幂，且 ≥ 单帧 258B） */
#define SRV_COM_SLV_INPUT_BUF_SIZE (512U)

/** @brief parser/packer 输出缓冲（需容纳总线最大帧） */
#define SRV_COM_SLV_OUTPUT_BUF_SIZE (SRV_COM_SLV_BUS_MAX_FRAME_LEN + 32U)

/** @brief 单次 feed 最多解析的帧数（防止单次调用占用主循环过长） */
#define SRV_COM_SLV_MAX_PARSE_PER_FEED (4U)

/** @brief 错误日志限频窗口 (ms) */
#define SRV_COM_SLV_ERR_LOG_PERIOD_MS (1000U)
...
static void com_reply_err(srv_com_slv_err_code_t code);
...
void srv_com_slv_init(const srv_com_slv_config_t* config)
{
    ...
    if (!config || !config->read_data || !config->ctrl || !config->send_frame) {
        SRV_COM_SLV_LOG_E("初始化失败: config/read_data/ctrl/send_frame 缺失");
        return;
    }
...
void srv_com_slv_deinit(void)
{
    ...
    SRV_COM_SLV_LOG_I("从机协议服务反初始化完成");
}
...
void srv_com_slv_rx_feed(const uint8_t* data, uint32_t len)
{
    ...
    while (parsed < SRV_COM_SLV_MAX_PARSE_PER_FEED) {
        ...
        if (err == PROTOCOL_PARSER_OK) {
            ...
            if (data_len > SRV_COM_SLV_MAX_PAYLOAD_LEN) {
                parsed++;
                continue;
            }
...
static void com_handle_frame(uint8_t cmd, const uint8_t* payload, uint8_t plen)
{
    ...
    uint8_t reply_len = 0;
    uint8_t reply_cmd = (uint8_t)(cmd | SRV_COM_SLV_CMD_REPLY_FLAG);

    switch (cmd) {
    case SRV_COM_SLV_CMD_READ_STATUS: {
        srv_com_slv_report_t report;
    ...
    case SRV_COM_SLV_CMD_READ_VOLT: {
        srv_com_slv_report_t report;
        memset(&report, 0, sizeof(report));
        if (s_read_data) {
            s_read_data(&report);
        }
        reply_payload[0] = (uint8_t)(report.aux_mv & 0xFFU);
        reply_payload[1] = (uint8_t)(report.aux_mv >> 8);
        reply_payload[2] = (uint8_t)(report.motor_mv & 0xFFU);
        reply_payload[3] = (uint8_t)(report.motor_mv >> 8);
        reply_payload[4] = (uint8_t)(report.lsd1_mv & 0xFFU);
        reply_payload[5] = (uint8_t)(report.lsd1_mv >> 8);
        reply_payload[6] = (uint8_t)(report.lsd2_mv & 0xFFU);
        reply_payload[7] = (uint8_t)(report.lsd2_mv >> 8);
        reply_len = 8U;
        break;
    }
    case SRV_COM_SLV_CMD_READ_TEMP: {
        srv_com_slv_report_t report;
        memset(&report, 0, sizeof(report));
        if (s_read_data) {
            s_read_data(&report);
        }
        reply_payload[0] = (uint8_t)(report.mcu_temp_x100 & 0xFFU);
        reply_payload[1] = (uint8_t)((uint16_t)report.mcu_temp_x100 >> 8);
        reply_payload[2] = (uint8_t)(report.vdda_mv & 0xFFU);
        reply_payload[3] = (uint8_t)(report.vdda_mv >> 8);
        reply_len = 4U;
        break;
    }
...
static void com_reply_err(srv_com_slv_err_code_t code)
{
    ...
    com_reply(SRV_COM_SLV_CMD_ERR, &code_b, 1U);
}
...
{
    if (s_initialized
        && (uint32_t)(millis() - s_last_err_log) >= SRV_COM_SLV_ERR_LOG_PERIOD_MS) {
        s_last_err_log = millis();
        SRV_COM_SLV_LOG_W("%s: err=%d", tag, (int)err);
    }
}
