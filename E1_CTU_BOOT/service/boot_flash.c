...
static ring_storage_error_t boot_meta_ring_open(boot_flash_context_t* ctx);
...
/**
 * @brief 打开 metadata ring_storage（init + register，不做 load/save）
 * @return ring_storage 错误码
 */
static ring_storage_error_t boot_meta_ring_open(boot_flash_context_t* ctx)
{
    const ring_storage_config_t cfg = {
    ...
    ring_storage_error_t e = ring_storage_init(&ctx->meta_storage, &cfg);
    if (e != RING_STORAGE_OK && e != RING_STORAGE_ERROR_NO_VALID_FRAME) {
        return e;
    }
    return ring_storage_register(&ctx->meta_storage, "meta",
        &ctx->meta_cache, sizeof(ctx->meta_cache));
}

boot_flash_error_t boot_flash_peek_metadata(boot_flash_context_t* ctx,
    boot_metadata_t* metadata)
{
    ...
    if (!ctx->initialized) {
        ...
        ring_storage_error_t e = boot_meta_ring_open(ctx);
        if (e != RING_STORAGE_OK && e != RING_STORAGE_ERROR_NO_VALID_FRAME) {
            return BOOT_FLASH_ERROR_ERASE;
        }
        e = ring_storage_load(&ctx->meta_storage);
        if (e == RING_STORAGE_ERROR_NO_VALID_FRAME) {
            memset(&ctx->meta_cache, 0, sizeof(ctx->meta_cache));
            ctx->meta_cache.magic = 0U; /* 无有效 metadata：调用者据 magic 判定 */
        } else if (e != RING_STORAGE_OK) {
