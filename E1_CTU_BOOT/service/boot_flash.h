/**
 * @file    boot_flash.h
 * @brief   Flash 分区管理 — 双 A/B 分区 + Metadata 页（E1_CTU_BOOT 本地实现）
 * @attention
 *
 * 本项目（Bootloader）本地持有 boot 分区/metadata 逻辑，不依赖 public_layer/service。
 * 由公共 boot 栈移植而来，默认按 E1_CTU_BOOT 布局：
 *   BOOT 0x08000000 (32KB) / AppA 0x08008000 (96KB) / AppB 0x08020000 (96KB)
 *   / 保留 24KB / Metadata 0x0803E000 (8KB)
 * 与兄弟工程 App 侧升级标志写入共享同一区域与字节契约（见 docs/boot_485_ymodem.md）。
 * 封装 hal_flash，提供分区维度的擦除/写入/校验操作。
 *
 * 分区大小/元数据偏移为编译期可覆写宏（#ifndef 默认值）；板级经编译定义传入布局。
 * 单分区模式：编译定义 BOOT_SINGLE_PARTITION 后仅 App A 一个槽位。
 * 默认双分区（不定义 BOOT_SINGLE_PARTITION）。
 */
...
/**
 * @brief 只读读取 Metadata（不修改、不写 Flash、不累加启动次数）
 * @param ctx 上下文指针
 * @param metadata 输出：Metadata 结构体
 * @return 操作结果
 * @note  供 App 侧信息查询（0x07）使用：不会像 boot_flash_init 那样
 *        reboot_counts++ 并保存。首次调用会初始化 ring 并加载缓存。
 */
