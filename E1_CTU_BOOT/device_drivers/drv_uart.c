 ...
 *  Boot 用途需容纳 YMODEM 1KB 数据包整包突发（≥1024）；取 2048 留整包余量。 */
#define DRV_UART_RX_BUF_SIZE (2048U)

/** @brief 错误日志聚合窗口 (ms)：窗口内的错误只在首次打印，附累计次数 */
#define DRV_UART_ERR_LOG_PERIOD_MS (1000U)

/** @brief TC 等待保护次数（TX 移位寄存器清空有界自旋，见 drv_uart_tx_shift_wait） */
#define DRV_UART_TX_TC_GUARD_LOOPS (1000U)
...
void drv_uart_tx_flush(drv_uart_channel_t ch)
{
    ...
    /* 主循环驱动：DMA 空闲且有帧时启动，TxCplt 清忙后由本函数下轮续发。
       单次调用仅发一帧即返回，避免长时间占用主循环（帧间隔由调用周期保证）。 */
    if (inst->huart->gState == HAL_UART_STATE_READY
        && !inst->tx_busy
        && !msg_fifo_empty(&inst->tx_fifo)) {
        drv_uart_tx_frame_t frame;
...
{
    if (ch >= DRV_UART_CH_NUM || !s_inst[ch].initialized
        || s_inst[ch].huart == NULL) {
        return true; /* 无有效实例：无字节在发，视作已就绪 */
    }

    UART_HandleTypeDef* huart = s_inst[ch].huart;
    uint32_t guard = DRV_UART_TX_TC_GUARD_LOOPS;
    while ((huart->Instance->SR & USART_SR_TC) == 0U) {
        if (--guard == 0U) {
            return false; /* 超时（异常场景），由调用方决定后续处理 */
        }
    }
    return true;
}
...
static bool drv_uart_rx_start(drv_uart_inst_t* inst)
{
    ...
    /* 关闭 USART 错误中断(EIE)：NE/FE/ORE 不再触发 HAL 阻塞式中止。
       F103 在 DMA 接收下任一错误(含噪声 NE)都会被 HAL 当阻塞错误整体中止，
       中断整帧 YMODEM 接收；实际噪声/个别坏字节由数据块 CRC + 主机重传兜底，
       而 DMA/总线级硬错误仍经 DMA 错误路径触发 error_cb 自恢复。 */
    __HAL_UART_DISABLE_IT(inst->huart, UART_IT_ERR);
 ...
 * @note  DMA circular：IDLE/HT 事件后硬件继续接收，仅同步 kfifo 读指针，不重启。
 *        HT(半满) 事件同样需要同步——否则连续突发（如 YMODEM 1KB 整包）会覆盖
 *        未消费数据；消费方在主循环按块粒度读取。
 */
static void drv_uart_rx_event_cb(UART_HandleTypeDef* huart, uint16_t Size)
{
